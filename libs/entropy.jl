...
function node_prob(n::Integer,
                   fp::Integer,
                   fn::Integer,
                   α::Float64,
                   β::Float64)
    bin = ((x, fx, ξ) ->
        binomial(big(n), fx) * ξ ^ fx * (1 - ξ) ^ (n - fx)) 
...
function prob_distribution(error_m::Tuple{NamedArray, NamedArray},
    ...
    result = NamedArray(zeros(s[1], s[2]),
                        (allnames(error_m[1])[1],
                         allnames(error_m[1])[2]))
    ...
    for i in 1 : s[1]
        for j in 1 : s[2]
            ...
            result[i,j] = node_prob(n[current],
                                    Int(error_m[1][i,j]),
                                    Int(error_m[2][i,j]),
                                    α,
                                    β)
...
function entropy_array(error_p::NamedArray)
    result =
        NamedArray([entropy(error_p, i) for i in allnames(error_p)[1]],
                   (allnames(error_p))[1])
    return result
end
